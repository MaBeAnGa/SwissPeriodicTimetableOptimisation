...
def _run_sequential_year_subprocesses(args: argparse.Namespace, years: list[str], parent_log_path: Path) -> None:
    ...
    for year in years:
        ...
        if args.resume_accepted_csv is not None:
            cmd.extend(["--resume-accepted-csv", _arg_path(args.resume_accepted_csv)])
...
def _merge_sequential_year_outputs(
    *,
    ...
    _merge_named_csvs(
        child_dirs,
        "Step3_AllSuggestedTimetableChanges_2026_2035.csv",
        parent_work_dir / "Step3_AllSuggestedTimetableChanges_2026_2035.csv",
        ["sequential_run_year", "step3_acceptance_rank", "step3_resume_new_rank", "status", "year", "line", "delta_min", "physical_shift_id"],
    )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional CSV of already accepted Step 3 suggestions. These rows are applied to the "
            "working timetable before the resumed search starts and are included in the combined "
            "reference-path base effect."
        ),
    )
    ...
    _log(log_path, f"Step 3 resume/search start years={years}")
    ...
    if args.resume_accepted_csv is not None:
        _log(log_path, f"Resume accepted Step 3 CSV={Path(args.resume_accepted_csv).expanduser().resolve()}")
    ...
    resume_event_keys: set[tuple[str, str, str, str, int]] = set()
    if args.resume_accepted_csv is not None:
        resume_rows, resume_ids, resume_event_keys = _load_prior_accepted(
            Path(args.resume_accepted_csv).expanduser().resolve(),
            year_set,
        )
    _log(log_path, f"Loaded already accepted Step 3 resume rows={len(resume_rows)} ids={len(resume_ids)}")

    blocked_ids = set(prior_ids) | set(resume_ids)
    blocked_event_keys = set(prior_event_keys) | set(resume_event_keys)
    ...
    candidates, rejected_pool = _build_step3_candidate_pool(
        ...
        years=year_set,
        prior_ids=blocked_ids,
        prior_event_keys=blocked_event_keys,
        max_shift_min=max(1, int(args.max_shift_min)),
    ...
    resume_applied_rows: list[dict[str, Any]] = []
    if resume_rows:
        _log(log_path, f"Applying {len(resume_rows)} already accepted Step 3 rows before resuming")
        for resume_rank, resume_row in enumerate(resume_rows, start=1):
            ok, new_working_rows, info = _try_apply_candidate(
                raw_row=resume_row,
                rank=len(prior_rows) + resume_rank,
                rows=working_rows,
                segments=segments,
            ...
            if not ok:
                raise RuntimeError(
                    "Could not apply resume Step 3 row "
                    f"rank={resume_rank} physical_shift_id={resume_row.get('physical_shift_id')} "
                    f"reason={info.get('reason', info.get('status', 'unknown'))}"
                )
            working_rows = new_working_rows
            seed_row = dict(resume_row)
            seed_row.setdefault("status", "accepted_step3_resume_seed")
            seed_row["step3_resume_seed_row"] = "1"
            seed_row["step3_resume_new_row"] = "0"
            seed_row["resume_seed_apply_status"] = str(info.get("status", ""))
            seed_row["resume_seed_matched_cells"] = int(info.get("matched_cells", 0))
            seed_row["resume_seed_changed_cells"] = int(info.get("changed_cells", 0))
            resume_applied_rows.append(seed_row)
        _log(log_path, f"Applied resume base rows={len(resume_applied_rows)}")
    ...
    used_ids: set[str] = set(blocked_ids)
    used_event_keys: set[tuple[str, str, str, str, int]] = set(blocked_event_keys) if int(args.block_step2_event_keys) else set()
    current_combined_saved = 0.0
    current_combined_summary: dict[str, Any] = {}
    if resume_applied_rows:
        _log(log_path, "Computing combined reference-path base effect for resume rows")
        if reference_path_cache:
            current_combined_summary = _scan_combined_from_reference_cache(
                accepted_rows=resume_applied_rows,
        ...
        current_combined_saved = _safe_float(current_combined_summary.get("current_route_exact_pax_minutes_saved"), 0.0)
        _log(log_path, f"Resume base combined reference-path saving={_fmt(current_combined_saved, 3)}")
    ...
    while remaining_by_id:
        ...
        base_plus_new = resume_applied_rows + accepted
        _log(log_path, f"Iteration {iteration}: scoring remaining candidate variants={len(remaining)} resume_base={len(resume_applied_rows)} new_accepted={len(accepted)}")
        if reference_path_cache:
            scores = _score_candidates_from_reference_cache(
                candidates=remaining,
                accepted_rows=base_plus_new,
                path_cache=reference_path_cache,
                station_min_transfer=station_min_transfer,
                log_path=log_path,
            )
        ...
        for rank, row in enumerate(scored, start=1):
            ...
            ok, trial_rows, info = _try_apply_candidate(
                raw_row=row,
                rank=len(prior_rows) + len(resume_applied_rows) + len(accepted) + 1,
                rows=working_rows,
                segments=segments,
                row_to_segment=row_to_segment,
                year_col_indices=year_col_indices,
            ...
            if int(args.combined_check_each_accept):
                ...
                if reference_path_cache:
                    combined_summary = _scan_combined_from_reference_cache(
                        accepted_rows=resume_applied_rows + accepted + [row],
                        path_cache=reference_path_cache,
                        station_min_transfer=station_min_transfer,
                        log_path=log_path,
                    )
            ...
            accepted_row = {
                "step3_acceptance_rank": len(resume_applied_rows) + len(accepted) + 1,
                "step3_resume_new_rank": len(accepted) + 1,
                "status": "accepted_step3_resume_suggestion",
                "step3_resume_seed_row": "0",
                "step3_resume_new_row": "1",
    ...
    if (resume_applied_rows or accepted) and not current_combined_summary:
        if reference_path_cache:
            current_combined_summary = _scan_combined_from_reference_cache(
                accepted_rows=resume_applied_rows + accepted,
                path_cache=reference_path_cache,
                station_min_transfer=station_min_transfer,
                log_path=log_path,
            )
    ...
    all_suggested_path = work_dir / "Step3_AllSuggestedTimetableChanges_2026_2035.csv"
    ...
    preferred_fields = [
        ...
        "step3_resume_new_rank",
        "step3_resume_seed_row",
        "step3_resume_new_row",
        "status",
        "year",
        "line",
        "delta_min",
    ...
    all_accepted_rows = resume_applied_rows + accepted
    _write_rows_union(suggested_path, accepted, preferred_fields)
    _write_rows_union(all_suggested_path, all_accepted_rows, preferred_fields)
    ...
    resume_by_year = Counter(str(row.get("year", "")) for row in resume_applied_rows)
    all_by_year = Counter(str(row.get("year", "")) for row in all_accepted_rows)
    summary = {
        "created_at": _now(),
        "years": years,
        ...
        "resume_seed_step3_rows": len(resume_applied_rows),
        "resume_seed_by_year": dict(sorted(resume_by_year.items())),
        "accepted_new_step3_suggestions": len(accepted),
        "accepted_step3_suggestions": len(accepted),
        "total_step3_rows_after_resume": len(all_accepted_rows),
        "accepted_by_year": dict(sorted(accepted_by_year.items())),
        "all_step3_rows_by_year": dict(sorted(all_by_year.items())),
        ...
        "output_all_suggested_csv": str(all_suggested_path),
    ...
    _log(log_path, f"Wrote new resume suggestions: {suggested_path}")
    _log(log_path, f"Wrote all Step 3 suggestions after resume: {all_suggested_path}")
