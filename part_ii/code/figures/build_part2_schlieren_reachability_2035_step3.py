...
YEAR = '2035'
ORIGIN = 'Schlieren'
OUT_PDF = FIG_DIR / 'part2_schlieren_reachability_2035.pdf'
OUT_SVG = FIG_DIR / 'part2_schlieren_reachability_2035.svg'
...
def load_reachability() -> dict:
    ...
    service.optimized_step2_root = (
        Path.home()
        / 'od_fast_state'
        / 'step3_local_outputs'
        / 'STEP3_BUILD_LOCAL_FINAL'
        / 'row_states'
        / 'nondominated_cells_state_STEP3_FINAL'
    )
...
def draw_legend_pdf(c: canvas.Canvas, x: float, y: float):
    ...
    c.drawString(x, y + 42, 'Fastest travel time from Schlieren')
    x0 = x
    yy = y + 24
    item_gap = 49
...
def create_pdf(reachability: dict, boundaries: dict, geom: dict):
    ...
    c.setTitle('Schlieren reachability before and after optimization')
    c.setFillColor(hex_color('#fbf8f1'))
    c.rect(0, 0, PAGE_W, PAGE_H, fill=1, stroke=0)
    draw_text(c, 'Reachability from Schlieren: baseline and post-optimization', PAGE_MARGIN_X, PAGE_H - PAGE_MARGIN_Y - 17, 'Helvetica-Bold', 15.0, colors.Color(17 / 255, 19 / 255, 21 / 255))
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {view_w:.0f} {view_h:.0f}" width="{view_w:.0f}" height="{view_h:.0f}">
  ...
  <text class="title" x="0" y="30">Reachability from Schlieren: baseline and post-optimization</text>
